...
class Sigmoid(Function):
    ...
    def backward(ctx: Context, grad_output: Tensor) -> Tensor:
        # TODO: Implement for Task 2.4.
        (t,) = ctx.saved_values
        ones = tensor(1.0, backend=t.backend)
        neg = t.f.neg_map(t)
        _t = t.f.add_zip(ones, neg)
        _grad = t.f.mul_zip(t, _t)
        return grad_output.f.mul_zip(_grad, grad_output)
...
class Permute(Function):
    ...
    def forward(ctx: Context, a: Tensor, order: Tensor) -> Tensor:
        ...
        tensor_data = a._tensor.permute(*order_list)
    ...
    def backward(ctx: Context, grad_output: Tensor) -> Tuple[Tensor, float]:
        ...
        tensor_data = grad_output._tensor.permute(*np.argsort(order_list))
        return (
            minitorch.Tensor(
                tensor_data, backend=grad_output.backend
            ), 
            0.0,
        )
